...
function DiscordError() {
  ...
  useEffect(() => {
    if (error === 'discord_denied') toast.error('Cancelaste el login con Discord.')
    if (error === 'discord_failed') toast.error('Error al conectar con Discord. Intenta de nuevo.')
  }, [error])
  return null
}
...
export default function LoginPage() {
  ...
  return (
    <>
    <Suspense><DiscordError /></Suspense>
    <div className="relative min-h-screen overflow-hidden bg-background flex items-center justify-center p-4">
      {/* Ambient glow */}
      <div
        className="pointer-events-none absolute inset-0"
        style={{ background: 'radial-gradient(ellipse 80% 60% at 50% 50%, rgba(180,140,30,0.07) 0%, transparent 65%), radial-gradient(ellipse 50% 40% at 100% 0%, rgba(99,58,180,0.08) 0%, transparent 60%)' }}
      />

      {/* Card */}
      <div className="animate-slide-up relative z-10 w-full max-w-md rounded-xl border border-amber-400/15 bg-card/90 p-8 shadow-[0_0_50px_oklch(0.085_0.018_265/0.8),0_0_25px_oklch(0.76_0.165_82/0.06)] backdrop-blur-sm">
